"""Edfali / Tadawul Cash (مصرف التجارة والتنمية)."""
...
class EdfaliGateway(LibyanGatewayAdapter):
    slug = "edfali"
    ...
    def verify_webhook_signature(self, payload: bytes, signature: str) -> bool:
        if not settings.edfali_webhook_secret:
            return settings.gateway_sandbox_mode
        return hmac_compare(sign_payload(settings.edfali_webhook_secret, payload), signature)

    def parse_webhook_status(self, payload: dict) -> tuple[str, str]:
        ref = payload.get("order_ref") or payload.get("ref", "")
        ok = payload.get("result") in {"OK", "success", "completed"}
