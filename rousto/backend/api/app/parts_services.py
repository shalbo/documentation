...
def part_out(part: Part, locale: str) -> dict:
    return {
        "id": part.id,
    ...
    for part in parts:
        if make or model:
            ...
            if not _matches_vehicle(compat, make, model):
                continue
        results.append(part_out(part, locale))
    return results
...
def get_part_detail(db: Session, part_id: uuid.UUID, locale: str) -> dict | None:
    ...
    data["stock_total"] = sum(row.qty_available for row in inventory)
    data["vendors_in_stock"] = len(inventory)
    return data
