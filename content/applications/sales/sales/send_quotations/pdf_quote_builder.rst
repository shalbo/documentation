...
The PDF Quote Builder groups header pages, product descriptions, the price(s), and footer pages
to create a detailed quote. It can also inject dynamic texts in the PDF to personalize the offer
for the customer.
...
.. note::
   ...
   different headers/footers on a quotation template(s).
...
While creating custom PDFs for quotes, use *PDF form* with components (also known as, Dynamic Texts)
for Odoo to auto-fill the PDF content with information related to the quote from the Odoo database,
like names, prices, etc.

Dynamic text values are form components (text inputs) that can be added in a PDF file, and Odoo
automatically fills it in with information related to the quote. Odoo fills these components
according to their name. Doing so streamlines the writing of personalized quotations.
...
greyed-out until a document is uploaded. Once a PDF has been uploaded, the :guilabel:`Name` field is
